...
def grad_train(
    ...
    wmode = 'disabled' if args.debug else None
    scher = ExponentialLR(learner.optim, gamma=0.92)
    args.lr_gamma = 0.92
    wandb.init(project=projn, entity='robehind', name=name,
               config=args, dir=args.exp_dir, mode=wmode)
    ...
    while steps < args.train_steps:
        ...
        if steps >= print_gate_steps:
            ...
            for i in [0, 1, 2]:
                ss = 'Mat'+str(i)
                out_data[ss] = wandb.Histogram(
                    getattr(learner.model, ss).detach().cpu().numpy())
            wandb.log(out_data, step=steps)
